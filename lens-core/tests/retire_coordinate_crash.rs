//! `VectorStore::retire_coordinate` CRASH-RECOVERY path (M4 Phase 4b, Step 8 / R3).
//!
//! This is the ONE crash test for `retire_coordinate`, isolated in its OWN test
//! binary (separate process) because it ARMS the process-global crash seam
//! `CRASH_AFTER_RETIRE_STALE_BEFORE_LANCE_DROP` (a `static AtomicBool`). The
//! sibling `retire_coordinate.rs` tests run in parallel in a single binary and
//! also call `retire_coordinate`; if the flag were armed in that shared process a
//! sibling could observe/consume it, producing a nondeterministic failure under
//! CI parallelism. Mirroring the `reembed_backend_switch_crash.rs` precedent, the
//! crash test lives alone here so the flag never leaks across tests.
//!
...
//! shares the engine's pool + lancedb root (same `data_dir`).
