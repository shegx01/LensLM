...
/// Wraps 16 kHz mono f32 PCM in an in-memory 16-bit PCM WAV. Sample mapping is
/// [`crate::f32_sample_to_i16`]; the header carries the exact data length.
...
pub fn pcm_to_wav(pcm: &[f32], sample_rate: u32) -> Result<Vec<u8>, LensError> {
    ...
    for &s in pcm {
        out.extend_from_slice(&crate::f32_sample_to_i16(s).to_le_bytes());
    }
    Ok(out)
}
