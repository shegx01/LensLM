...
/// `llm_inline` / `dedicated_model`), so an existing `config.json` round-trips.
/// Per Decision I, `DedicatedModel` is a config+trait stub in Phase 3 and behaves
/// EXACTLY like [`LlmInline`](CorefStrategy::LlmInline) at runtime (no `ort` dep
/// is pulled).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum CorefStrategy {
    /// No coref hint — the prefix is doc/section context only.
    None,
    /// Inline coref hint composed in the same LLM pass (the default).
    #[default]
    LlmInline,
    /// Reserved upgrade (Decision I): falls back to `LlmInline` in Phase 3.
    DedicatedModel,
}
...
impl CorefStrategy {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::DedicatedModel => "dedicated_model",
        }
    }

    /// Parses the persisted/config string. An unrecognized value defaults to
    /// [`LlmInline`](CorefStrategy::LlmInline) (the conservative Phase-3 default),
    /// so a future config value never panics the worker.
    pub fn from_config(value: &str) -> Self {
        match value {
            "none" => Self::None,
            "dedicated_model" => Self::DedicatedModel,
            // "llm_inline" and any unknown value → the default.
            _ => Self::LlmInline,
        }
    }

    /// Whether this strategy contributes an inline coref hint to the prefix.
    /// `DedicatedModel` falls back to `LlmInline` in Phase 3 (Decision I).
    fn applies_inline_hint(&self) -> bool {
        matches!(self, Self::LlmInline | Self::DedicatedModel)
    }
}
...
///
/// Shape: `"[Document: {summary}] [Section: {section_path}] {coref_hint} "`.
/// Each clause is omitted when its input is empty, so a skipped/non-prose chunk
/// (empty summary) still gets a `[Section: …]`-only prefix (Decision B). The
/// trailing space separates the prefix from the body.
pub fn compose_prefix(doc_summary: &str, section_path: &str, coref: CorefStrategy) -> String {
    ...
    if coref.applies_inline_hint() && !summary.is_empty() {
        // The inline-coref hint anchors pronouns to the document's named entities;
        // the LLM-derived summary already carries those, so the hint references it
        // rather than fabricating a separate coref payload (kept cheap — Decision
        // B: prefix even for skipped kinds is "near-zero cost").
        clauses.push("[Resolve pronouns using the document context above]".to_string());
    }
    if clauses.is_empty() {
        String::new()
    } else {
...
mod tests {
    ...
    fn prefix_omits_empty_clauses_but_keeps_section_for_skipped() {
        ...
        // No inline coref hint without a summary to anchor it.
        assert!(!p.contains("Resolve pronouns"));
    }

    #[test]
    fn prefix_adds_inline_coref_hint_under_llm_inline() {
        let p = compose_prefix("ctx", "S", CorefStrategy::LlmInline);
        assert!(p.contains("Resolve pronouns"));
        // DedicatedModel falls back to LlmInline (Decision I).
        let p2 = compose_prefix("ctx", "S", CorefStrategy::DedicatedModel);
        assert!(p2.contains("Resolve pronouns"));
        // None gets no hint.
        let p3 = compose_prefix("ctx", "S", CorefStrategy::None);
        assert!(!p3.contains("Resolve pronouns"));
    }
